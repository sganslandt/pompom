define(['jquery', 'taskList', 'modal', 'notify'], function ($, taskList, modal, notify) {
    ...
    function bindTaskModal(task) {
        var title = $(task).find('h3').html(),
            totalPoms = $(task).find('ol li').length,
            freshPoms = $(task).find('ol .fresh').length,
            content = "<p>Pomodoros left: <span class='pom-left'>" + freshPoms + " of " + totalPoms + "</span></p> \
        ...
        modal.create(title, content, 'additionalPomodoros');
